...
- [ ] 1.1 In `App.css`, add `text-align: center` to `.chip-label` (and
...
- [ ] 1.2 Manual: open a facility detail card, confirm "Power," "Capital
      cost," and "Impact radius" chips all look visually even regardless
      of label wrap

## 2. Grid renewables caveat

- [ ] 2.1 Confirm via a quick script over `backend/data/datacenters.json`
...
- [ ] 2.2 Add a brief inline caveat (e.g. a `title` tooltip, or a small
...
- [ ] 2.3 Do not change `renewable_pct`'s underlying value or the
...
- [ ] 3.1 In `CompareModal.jsx`, add local state tracking whether the
      search input is focused (or has a non-empty query)
- [ ] 3.2 Render `.compare-search-results` only when that state is true;
      `.compare-chip-list` keeps rendering unconditionally regardless of
      focus
- [ ] 3.3 Hide the dropdown on blur, but only after a short delay (or via
...
- [ ] 3.4 Update `CompareModal.test.jsx`: dropdown is hidden on initial
...
- [ ] 4.1 In `App.css`, bump `.region-metric-btn` (currently 11px) and
...
- [ ] 4.2 Check `.region-metric-btn--active`/`.region-basis-btn--active`
      still look correct at the new size (no layout shift/overflow in the
      switcher row)
- [ ] 4.3 Manual: open Region Scorecard in both dark and light theme,
...
- [ ] 5.1 In `ScenarioPanel.jsx`/`App.css`, unify `.scenario-active-actions`
...
- [ ] 5.2 Confirm the result container only appears once a preset has
      been applied (existing `activePresetId`/`data` gating), so the
      "here's your result" framing doesn't show prematurely
- [ ] 5.3 Update `ScenarioPanel.test.jsx` if markup/selectors changed for
      the actions/totals grouping

## 6. Verification

- [ ] 6.1 `cd backend && pytest` passes (no backend changes expected, but
      confirm nothing regressed)
- [ ] 6.2 `cd frontend && npm test` passes
- [ ] 6.3 Manual pass through all five fixes in both dark and light theme:
