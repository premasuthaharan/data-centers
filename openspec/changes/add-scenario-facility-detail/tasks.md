...
- [ ] 2.2 When `scenarioDc` is present but a given value is unchanged from
...
- [ ] 2.3 Add a small header/badge on the card when scenario data is being
      shown (e.g. "Under: Grid Decarbonization") so it's clear the card
      isn't just displaying live baseline data

## 3. Tests

- [ ] 3.1 `DataCenterCard.test.jsx` (new, if one doesn't already exist):
...
- [ ] 3.2 Update/add an `App.jsx` test (if App-level tests exist by then)
...
- [ ] 4.2 Manual: apply a scenario, click a facility marker, confirm the
