...
- [x] 1.1 List every `font-size` declaration in `App.css` that applies to
...
- [x] 2.1 Bump label/eyebrow tier 10px → 12px:
      `.dc-section-label`, `.chip-label`, `.scenario-totals-heading`
- [x] 2.2 Bump secondary/meta tier 11px → 13px:
...
- [x] 2.3 Bump primary/body tier 12px → 14px:
      ...
      `.compare-hint`, `.compare-table` — confirmed `.app-subtitle` is
      exclusively map title overlay chrome, left unchanged (out of scope)
- [x] 2.4 Bump title tier proportionally: `.dc-detail-name` 15px → 16px,
      `.scenario-panel-title` 15px → 16px, `.compare-modal-title` 15px →
      16px, `.chip-val` 13px → 15px

## 3. Layout adjustments (only if needed)

- [x] 3.1 After applying the new sizes, visually check `.stat-row` and
      `.scenario-totals-row` (label + value on one line) for cramped
      wrapping; widen `.detail-panel-wrapper` from `380px` only if
      needed, not preemptively — verified via screenshots, no wrapping;
      380px width left unchanged
- [x] 3.2 Check `.dc-detail-stats` stat chips (`.dc-stat-chip`) still fit
      three across without text truncation at the new sizes — confirmed
      via screenshot of `DataCenterCard`

## 4. Verification

- [x] 4.1 `cd frontend && npm test` passes (no component logic changed,
      but confirm no test asserts on specific pixel values)
- [x] 4.2 Manual: open each of the four panels (facility detail card,
      ...
      larger sizes — verified all four via headless-browser screenshots
      against the local dev server; text is larger, legible, and nothing
      wraps/clips/truncates
