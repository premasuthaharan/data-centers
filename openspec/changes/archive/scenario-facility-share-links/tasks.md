...
- [x] 1.1 Pass the currently selected facility id (`App.jsx`'s
      `selectedId`) into `ScenarioPanel` as a new prop (e.g.
      `selectedFacilityId`)
- [x] 1.2 `ScenarioPanel`'s `copyLink`: when `selectedFacilityId` is
...
- [x] 2.1 Pass the currently applied scenario's encode inputs (preset id +
      overrides, already available as `scenarioData.presetId` /
      `scenarioData.scenario` in `App.jsx`) into `DataCenterCard` as new
      props
- [x] 2.2 `DataCenterCard`'s `copyLink`: when a scenario is active, add
...
- [x] 3.1 Sequence the three mount-time concerns so a link with both
...
- [x] 3.2 When both `?scenario=` and `?facility=` decode successfully, set
...
- [x] 5.2 Manual: apply a scenario, click into a facility, copy the link
      ...
      flashing first
- [x] 5.3 Manual: repeat starting the copy from the scenario panel's
      "copy link" (with a facility selected underneath) — confirm the same
      combined URL and resulting behavior
- [x] 5.4 Manual: confirm scenario-only and facility-only share links (from
