## 1. Data table
- [ ] 1.1 Extend `GRID_DATA` (or add a parallel table) in `fetch_data.py`
      with `electricity_price_usd_per_kwh` and `water_liters_per_kwh` per
      country
- [ ] 1.2 Source reasonable per-country starting values (e.g. IEA
...
- [ ] 1.3 Keep an explicit documented default for countries not in the
      table (do not silently reuse the current global constants without
      labeling them as defaults)

## 2. Backend
- [ ] 2.1 Add `electricity_price_usd_per_kwh` and `water_liters_per_kwh`
      fields to each record when generating `datacenters.json`
- [ ] 2.2 Update `compute_impact()` in `logic.py` to read these fields off
...
## 3. Regeneration
- [ ] 3.1 Regenerate `datacenters.json` with the new fields populated for
      all 43 entries

## 4. Verification
- [ ] 4.1 Spot check that facilities in different countries now produce
      different water/cost estimates for the same `power_mw`
- [ ] 4.2 Confirm the fallback default still applies correctly for any
