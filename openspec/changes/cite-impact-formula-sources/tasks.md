## 1. Research
- [ ] 1.1 Find a source (or confirm none exists) for: PUE 1.3 assumption,
      ...
      renewable defaults
- [ ] 1.2 For any constant with no real authoritative source, explicitly
      label it in `SOURCES.md` as "internal heuristic, not externally
      sourced" rather than implying it's authoritative

## 2. Documentation
- [ ] 2.1 Create `backend/SOURCES.md` listing each constant, its value,
      where it's used (function/line), and its citation or heuristic label
- [ ] 2.2 Update the inline comments in `logic.py` (lines ~40-68) to
      reference `SOURCES.md` by name rather than making standalone claims

## 3. Verification
- [ ] 3.1 Confirm every numeric constant in `compute_impact()` has a
